...
写给：一澜、林湛
状态：设计已讨论确认，**尚未实现**，先存档，等排到日程再动手（不占用当前 memzep 骨架层的工期）

**完成计划弹出的成就卡，跟骨架层的事实资料卡是同一套地基（标题+内容+附件+时间线）**，设计讨论存档在 `docs/facts-card-and-batch-import-plan.md`，实现时两边应该共用同一套 UI 组件和后端字段，不要各做一套。
