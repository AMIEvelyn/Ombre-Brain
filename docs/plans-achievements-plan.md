...
**完成计划弹出的成就卡，跟骨架层的事实资料卡是同一套地基（标题+内容+附件+时间线）**，当前权威设计存档在 `docs/facts-model-v2-collection-redesign.md`（2026-07-16 更新：骨架层已经历一次模型重构，卡片改成收藏/歌单式的卡+文件夹多对多，之前指向 `facts-card-and-batch-import-plan.md` 的旧引用已不是当前设计），实现时两边应该共用同一套 UI 组件和后端字段，不要各做一套。
